...
class CustomDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        return TENSOR_IMAGE,label, self.img_paths[index]
...
def eval_for_confmat(validation_loader, model = model, device = device, criterion = None):
    ...
    with torch.no_grad():
        model.eval()
        for data_,target_, img_path in validation_loader:
            ...
            my_images.append(img_path)
            all_outputs.append(np.array(torch.nn.functional.softmax(outputs, dim=1)))
            total_true += true
            total += target_.size(0)

    validation_accuracy = round(100 * total_true / total,2)
    return actual, predicted, all_outputs, my_images
...
def plot_confusion_matrix(confusion_matrix, savepic):
    df_cm = pd.DataFrame(confusion_matrix, index = [i for i in ['NC', 'CC']],
                  columns = [i for i in ['NC', 'CC']])
